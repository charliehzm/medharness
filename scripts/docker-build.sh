...
readonly AVAILABLE_MCPS="phi-detector desensitize model-router audit-log a0-api ci-trigger internal-kb pm-bridge vector-db"
...
mcp_size_target_mb() {
  case "$1" in
    phi-detector|desensitize|model-router|audit-log|a0-api)
      echo 500
      ;;
...
main() {
  ...
  case "${mcp}" in
    phi-detector|desensitize|model-router|audit-log|a0-api|ci-trigger|internal-kb|pm-bridge|vector-db)
      ;;
    *)
