# MedHarness Unified Gateway · 统一合规网关架构

> **状态**：v0.6 纲领文档（draft）· 指导从 v0.5.0-edge "AI Coding 体系" 演进为 "LLM 流量合规网关"
> **维护**：技术委员会 + 合规委员会
...
**核心结论**：真正不可绕的闸门必须 **IDE 无关**，下沉到**网络层**。一个统一的 LLM 流量合规网关，同时服务开发期和生产期。

### 1.2 一句话定位

> 不管是 Claude Code 写代码，还是 Dify 跑生产工作流——只要它调 LLM，流量就必经同一个 MedHarness Gateway。上游千变万化，合规强制点只有一个。
...
| **Phase B** | Anthropic 协议 + **流式 SSE** + Console 流量监控 | Claude Code + 生产流式 + 可视化 | 流式边扫边转发 |
| **Phase C** | 多模态 PHI（影像/DICOM/OCR）+ RAG ingestion 脱敏 + 配置治理 | ComfyUI 影像 + Dify 知识库 + 配置审批 | 全新能力 · 最难 |
...
| Gap | 影响 | 阶段 |
|---|---|---|
| 协议归一化（OpenAI + Anthropic 双协议） | Phase A 核心 | A |
| 流式 SSE PHI 扫描（边扫边转发） | 生产系统大量用 streaming | B |
| 高并发性能（生产 QPS vs 5ms budget） | 生产延迟 | B |
| **多模态 PHI（DICOM/影像/OCR）** | **ComfyUI 医学影像扫不到** | **C（最难）** |
| RAG ingestion 脱敏（入库时 vs query 时） | Dify 知识库 PHI | C |
| 工作流多跳 PHI 一致性（map_id 跨节点） | Dify 多 LLM 节点串联 | C |
...
- **决策**：闸门强制点从"IDE hook / MCP 调用"下沉到"网络层 egress gateway（OpenAI/Anthropic 协议兼容 HTTP proxy）"· 服务开发期（Claude Code/Codex）+ 生产期（Dify/ComfyUI/自研）统一。
- **替代**：A) 继续 IDE hook（Codex/Dify 失效）· B) 每个上游单独适配（N×成本）· C) SDK 侵入（上游要改代码）
- **否决理由**：A 不通用；B 不可维护；C 上游零改造原则。network proxy 是唯一 IDE/平台无关的强制点。
- **实施约束**：复用 v0.5.0-edge 全部闸门内核 · nginx DMZ 做 proxy 入口 · egress firewall 封直连 · 配置变更落 audit · dashboard 数据边界（永不显示原始 PHI）

---

## 10. 一句话总结

> MedHarness Gateway 是一个 LLM 流量合规网关：开发者用 Claude Code/Codex，业务系统用 Dify/ComfyUI——它们只需把 base_url 指向 gateway，所有含 PHI 的 LLM 流量就在网络层被自动扫描、脱敏、路由、审计。一套部署，开发期生产期通吃，上游零改造，配置可视化，合规可追溯。
