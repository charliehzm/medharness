...
> **状态**：**定稿 v1（实现基线）** · 异构闸门 WAIVED（待 r3）。**已建** = web/ React18 脚手架 + A0 契约层（v0.7.0）+ `Sanitized<T>` 0-PHI 守卫；**待建** = 设计 token/组件/壳/各屏（F1.2–F3）。
...
endpoints.ts  ENDPOINTS(GET×8+POST×2) + CONFIG_SECTIONS(10) + buildPath()
...
- **读**（列表/详情）：经 A0 **管理只读代理** `GET /admin/{users|tokens|channels}`，**字段白名单**（只回 `id 哈希/角色/配额/数据等级/区域`，**禁 email/phone/display_name/备注**），过 `Sanitized<T>` 守卫。
