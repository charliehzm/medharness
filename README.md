...
>
> 一套面向医疗数据 SaaS / 数据中台公司的开源 AI Coding 控制平面。
> 它把 PHI 脱敏、模型 allowlist 路由、全量审计、红队演练和生产编排串成一条可落地的工程链路。
> 当前处于 v0.5.0-edge / Phase 3，安全与部署主线已经进入生产化阶段。
...
- 版本定位：v0.5.0-edge
- 主线进展：T1-T10 已闭合，T11 TLS 正在推进
- 生产能力：8 MCP Docker 镜像、生产 Compose、双网 DMZ、红队与审计链已就位
- 后续方向：TLS 双路径、备份恢复、离线打包、镜像注册表
...
- **审计不断链**：`audit-log` 记录工具、模型、Skill 调用证据。
- **安全不靠口号**：prompt-injection drill、recall gate、Docker build + scan gate 进 CI。
- **部署不是 demo**：8 MCP Dockerfile、`deploy/docker-compose.prod.yml`、nginx DMZ、TLS 双路径陆续落地。
...
# 5. production 预览（8 MCP + nginx + 双网）
...
| `audit-log` | production | WORM / hashchain / fallback audit 基础 |
...
- **T11**: TLS 双路径、443、证书检查与 self-signed 默认（进行中）
...
| 8 MCP server（v2 实现 + 生产化 Docker / Compose） | ✅ | ✅ |
